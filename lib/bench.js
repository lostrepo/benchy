...
async function web (name, cmd, args, port = 3000, cwd = '', config, pinning) {
  ...
  for (let r = 1; r <= runs; r++) {
    ...
    if (pinning) {
      if (config.nice) {
        httpd = run('nice', ['-n', config.nice, 'taskset', '--cpu-list', pinning[0], cmd, ...args])
      } else {
    ...
    for (let w = 1; w <= warmups; w++) {
      let wrk
      if (pinning) {
        wrk = run('taskset', ['--cpu-list', pinning[1], 'wrk', '-t', threads, 
          '-d', warmupDuration, '-c', conn, `http://127.0.0.1:${port}/`
        ])
      } else {
        wrk = run('wrk', ['-t', threads, '-d', warmupDuration, '-c', conn, 
          `http://127.0.0.1:${port}/`
        ])
      }
/*
...
async function flamegraph (name, cmd, args, cwd = '') {
  const pwd = Deno.cwd()
  if (cwd) Deno.chdir(cwd)
  const prog = run(cmd, args)
  if (cwd) Deno.chdir(pwd)
  const flame = run('flamegraph', [
    '--freq', '60000', '--deterministic', '--no-inline', '--palette', 'js', '-o', `${name}.svg`, '--pid', prog.pid
  ], Deno.cwd(), { stdout: 'null', stderr: 'null', stdin: 'null' })
  await prog.waitfor()
  await flame.waitfor(true)
  const result = { 
    prog: { status: prog.status.code }
  }
  result.flame = `${name}.svg`
  return result
}
...
export { http, websocket, versions, flamegraph }
